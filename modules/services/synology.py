...
# 첨부로 받아줄 최대 크기. 흘려보내므로 우리 서버의 메모리·디스크와는 무관하고,
# **저장소(supabase-storage)가 한 파일에 허용하는 크기**가 곧 한도다.
# 지금 그 값은 4GB(FILE_SIZE_LIMIT=4294967296) — 넘겨 보내면 413 으로 튕긴다.
# 저장소 한도를 올리면 이 값도 같이 올린다(둘이 갈리면 다 올린 뒤에 실패한다).
MAX_STREAM = 4 * 1024 * 1024 * 1024
