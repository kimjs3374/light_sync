-- ══════════════════════════════════════════════════════════════════════
-- ⚠ 2026-09-17 부터 **스키마 변경은 여기 적지 않는다.**
--
--   새 컬럼·새 표는 migrations/versions/ 에 리비전으로 만든다:
--       venv/bin/alembic revision -m "무엇을 왜"
--       ./scripts/db-upgrade.sh            (올린다 · 올리기 전 스키마 덤프)
--       ./scripts/db-upgrade.sh --dry-run  (뭘 할지 SQL 로 먼저 본다)
--
--   왜: 이 파일은 사람이 손으로 쳐야 한다. 남에게 파는 물건이 되면
--   업데이트할 때마다 남의 회사 DB 에 손으로 SQL 을 치게 되고, 한 번 틀리면
--   그 회사 메일이 멈춘다.
--
--   아래 내용은 **지나간 기록**이다. 지우지 않는다(왜 그렇게 만들었는지가 여기 있다).
-- ══════════════════════════════════════════════════════════════════════
