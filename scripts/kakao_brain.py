...
def ask_claude(question: str, resume_sid=None, new_sid=None, image_path=None, erp_user=None):
    ...
    if image_path:
        ...
        prompt = (
            ...
            f"(3) 차량이 확정되면 즉시 get_business_trips()으로 오늘 날짜 기준 출장을 조회한다(진행중·완료 모두 포함). "
            f"결과 중 ① 확정된 차량과 일치하고 ② 오늘 날짜 출장이고 ③ 사진을 보낸 본인(운전자)이 출장인원(members)에 포함된 건만 후보로 삼는다. "
            f"후보가 1건이면 묻지 말고 from_trip_id로 바로 연동해 write_preview_vehicle_log를 호출한다. "
            f"후보가 0건이면 목적지·출발지·목적을 사용자에게 되물어 채운다. "
