...
# ===================================================================
# 3-2. 품목별 거래 이력 + 단가 추이 (AJAX JSON)
# ===================================================================
@vendor_bp.route('/vendor/item-history')
@login_required
def item_history():
    """품목별 거래 이력 + 단가 추이 데이터 (JSON).
    자체 DB의 PurchaseOrderHistory/ReceivingHistory items_json에서 추출.
    """
    item_cd = (request.args.get('item_cd') or '').strip()
    if not item_cd:
        return jsonify({'error': '품목코드를 지정해주세요.'}), 400

    with get_db() as db:
        result = []

        # 발주 이력에서 품목코드 검색
        po_histories = db.query(PurchaseOrderHistory).all()
        for po in po_histories:
            items_list = _parse_items_json(po.items_json)
            for item in items_list:
                if item.get('item_cd') == item_cd:
                    result.append({
        ...
        # 입고 이력에서 품목코드 검색
        rcv_histories = db.query(ReceivingHistory).all()
        for rcv in rcv_histories:
            items_list = _parse_items_json(rcv.items_json)
            for item in items_list:
                if item.get('item_cd') == item_cd:
                    result.append({
                        'date': rcv.receive_date.strftime('%Y-%m-%d') if rcv.receive_date else '',
                        'type': 'RCV',
                        'vendor': rcv.vendor_name or '',
                        'item_cd': item_cd,
                        'spec': item.get('spec', ''),
                        'qty': float(item.get('qty', 0)),
                        'unit_price': float(item.get('unit_price', 0)),
        ...
        chart_data = {
            ...
            'po_prices': [r['unit_price'] if r['type'] == 'PO' else None for r in result],
            'rcv_prices': [r['unit_price'] if r['type'] == 'RCV' else None for r in result],
        }
